...
-- layout and style
local kTypes = pandoc.List({'consulting', 'study', 'report', 'comment'})
...
return {
  {
    ...
    Meta = function(meta)            
      if quarto.doc.is_format("pdf") then
        ...
        -- NOTE: (ML) Not sure if this is used
        ...
        -- NOTE: (ML) Not sure if this is used
        local byAffiliation = meta['by-affiliation']
        if byAffiliation ~= nil then
          for i, affiliation in ipairs(byAffiliation) do
            local prefix = {pandoc.RawInline("tex ","")};
            if i > 1 and i % 2 == 1 then
              prefix = {pandoc.RawInline("tex", "\\AND")}
            elseif i > 1 then
            ...
            affiliation['metadata']['latex-prefix'] = prefix
          end
        end
